...
func Exec(cfg Config) (err error) {
	if len(cfg.Scope) == 0 {
		cfg.Scope = "./"
	}
...
func matchFields(scope string, ident string, funcParams bool) (fields []MatchStruct) {
	...
	if err = executor.ExecFiles(scope, func(path string) (err error) {
		...
		dirPkg, _ := helpers.GetPathModPkg(filepath.Dir(path))
...
func ExecFields(cfg Option, fields []MatchStruct) (err error) {
	if err = helpers.FixFilepathByProjectDir(&cfg.Path); err != nil {
		return
	}

	removeDuplicate(fields)

	pathPkg, err := helpers.GetPathModPkg(filepath.Dir(cfg.Path))
	if err != nil {
		return
	}

	mounter, err := NewStructMounter(cfg.Path, cfg.Struct)
	if err != nil {
		return
	}
